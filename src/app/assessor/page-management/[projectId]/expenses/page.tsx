...
export default function AssessorProjectExpensesPage({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    void getAssessorProjectTabData(projectId, "expenses")
      .then((payload) => {
        if (cancelled) return;
        setData(payload);
      })
      .catch((e: unknown) => {
        if (cancelled) return;
        setError(e instanceof AuthApiError ? e.message : "Could not load expenses.");
        setData({});
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-semibold text-[#2f3a46]">Finance</p>
